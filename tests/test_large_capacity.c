...
/*
 * Test: Initialize a 16 GB geometry, verify L2P addressing,
 * write+read boundary LBAs through the full NVMe uspace path.
 *
 * Geometry: 8ch * 4chip * 2die * 2plane * 2048blk * 256pg * 4096B
 *         = 8*4*2*2*2048*256 = 67,108,864 pages
 *         = 256 GB raw NAND
 *         = ~16M usable LBAs at 4K with OP
 */

static void test_large_geometry_init(void)
{
    printf("\n=== Large Capacity Init (16 GB) ===\n");
    ...
    config.sssim_cfg.channel_count     = 8;
    config.sssim_cfg.chips_per_channel = 4;
    config.sssim_cfg.dies_per_chip     = 2;
    config.sssim_cfg.planes_per_die    = 2;
    config.sssim_cfg.blocks_per_plane  = 2048;
    config.sssim_cfg.pages_per_block   = 256;

    /* 16 GB = 4,194,304 LBAs at 4K */
    uint64_t total_lbas = 16ULL * 1024 * 1024 * 1024 / 4096;
    config.sssim_cfg.total_lbas = total_lbas;
    config.sssim_cfg.lba_size   = 4096;

    TEST_ASSERT(total_lbas == 4194304, "16 GB = 4,194,304 LBAs");

    int ret = nvme_uspace_dev_init(&dev, &config);
    TEST_ASSERT(ret == HFSSS_OK, "nvme_uspace_dev_init succeeds for 16 GB");
    ...
    TEST_ASSERT(id_ns.nsze >= total_lbas, "namespace size >= 16 GB LBAs");
...
static void test_boundary_io(void)
{
    ...
    uint64_t total_lbas = 4194304;  /* 16 GB */
    config.sssim_cfg.total_lbas = total_lbas;
    config.sssim_cfg.lba_size   = 4096;
...
{
    printf("========================================\n");
    printf("Large Capacity Tests (16 GB)\n");
    printf("========================================\n");
