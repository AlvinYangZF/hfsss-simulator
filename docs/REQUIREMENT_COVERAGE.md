...
| Algorithm Task Layer (FTL) | 22 | 19 | 1 | 2 | 0 | 86.4% | ↑ +6 (cmd state machine, retries, flow ctl, wear monitor, Error Log Page); FTL die-busy retry-spin replaced by per-die wait queue + completion-event-driven dispatch (`src/ftl/die_dispatcher.c`) — see `docs/superpowers/specs/2026-04-30-ftl-die-busy-waitqueue-design.md`; 012_seqwrite_verify latency root cause under investigation — dispatcher (PR #118) + FIFO ticket lock (PR #119) did not improve write latency (141.9 ms vs baseline 142.0 ms), per `docs/superpowers/specs/2026-04-30-ftl-die-busy-waitqueue-design.md` Section 9.2 |
...
| REQ-121 | Simulation Accuracy - NAND Latency Error | ✅ | **Scope of closure:** REQ-121 is read as "the simulator exposes a configurable NAND latency-error model"; the closure provides the mechanism, not a calibration report against any specific vendor device. `timing_model` carries a seeded LCG plus a `jitter_basis_points` field (±N/10000, capped at `TIMING_JITTER_MAX_BP` = ±20%, both `_Atomic` so enable/disable races against concurrent latency reads are well-defined). `timing_get_{read,prog,erase}_latency` apply jitter via a relaxed-CAS `advance_lcg`; `apply_jitter` acquires `jitter_basis_points` so a non-zero reading synchronizes-with the seed store that preceded it, and floor-clamps the result at `base/2` so a maximal negative draw cannot collapse latency to zero. `timing_model_enable_jitter(model, basis_points, seed)` arms the injector (relaxed seed store, release basis_points store); `timing_model_disable_jitter` or `basis_points=0` disarms. Tests: `tests/test_timing_jitter.c` covers disabled-by-default, ±basis_points bounding for read/prog/erase, byte-identical reproducibility across same-seeded models, divergence across different seeds, mean convergence to baseline within 0.5% over 20k samples, disable restoring baseline, the ±20% ceiling clamp, and NULL-safe no-op paths (111 assertions). **Follow-up (not gated on this REQ):** calibrate the ±basis_points envelope against reference-device variance data. |
...
| REQ-153 | Deterministic latency window (duty cycle) | ✅ | 3-phase (HOST_IO / GC_ALLOWED / GC_ONLY) cycle configured via `det_window_init`; `det_window_admit_host_io` / `det_window_admit_gc` enforce the window and record each admission/rejection into `struct det_window_stats` (per-phase counters, `phase_transitions` guarded by a `last_phase_valid` sentinel, separate `*_admitted_while_disabled` audit counters). `det_window_get_stats` / `det_window_reset_stats` expose the audit trail. **Production consumer:** `gc_run_mt` in `src/ftl/gc.c` snapshots an attachable `gc_admit_gc_fn` callback under `ctx->lock` and consults it each LBA-scan iteration; on rejection it bails the run and bumps `gc_ctx.det_window_rejects`. `det_window_gc_admit_adapter` (in `src/controller/det_window.c`) is the shim that bridges the callback to `det_window_admit_gc` without creating a link dep from libhfsss-ftl to libhfsss-controller. The NBD server (`src/vhost/hfsss_nbd_server.c`) attaches the adapter when `HFSSS_DET_WINDOW_{HOST,GC_ALLOWED,GC_ONLY}_PCT` + `_CYCLE_MS` env vars are supplied (absent / invalid config leaves the callback detached). Implements HLD_02 §11.3.3 "No GC page moves occur during HOST_IO window". Covered by `tests/test_qos.c::test_det_window_admit_stats` / `test_det_window_admit_first_non_host_io` / `test_det_window_admit_disabled` / `test_det_window_gc_admit_adapter` (stats-layer accounting) and end-to-end by `tests/test_gc_mt.c::test_gc_mt_respects_admit_callback_reject` (100% HOST_IO window → zero page moves + rejects counter advances on both `gc_ctx` and `det_window`; 100% GC_ALLOWED window → GC makes progress with the same adapter). |
