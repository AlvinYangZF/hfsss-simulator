...
| Algorithm Task Layer (FTL) | 22 | 19 | 1 | 2 | 0 | 86.4% | ↑ +6 (cmd state machine, retries, flow ctl, wear monitor, Error Log Page); FTL die-busy retry-spin replaced by per-die wait queue + completion-event-driven dispatch (`src/ftl/die_dispatcher.c`) — see `docs/superpowers/specs/2026-04-30-ftl-die-busy-waitqueue-design.md`; 012_seqwrite_verify latency target remains deferred to the cmd_engine FIFO ticket-lock follow-up (`docs/superpowers/specs/2026-05-08-cmd-engine-ticket-lock-design.md`) |
